...
!function (a, b, c) {
    function d(a, b) {
        ...
        if (c.opts = l.extend({}, e, b), "undefined" == typeof c.opts.pattern)throw new TypeError("Must provide a pattern");
        ...
        c.mLength = d.mLength, c.chars = d.chars, c.inpts = d.inpts, c.hldrs = {}, c.focus = 0, l.addListener(c.el, "keydown", function (a) {
            ...
            c._paste(a)
        }), c.opts.persistent && (c._processKey("", !1), c.el.blur(), l.addListener(c.el, "focus", function (a) {
            c._focus(a)
    ...
    d.prototype.resetPattern = function (a) {
        this.sel = k.get(this.el), this.val = this.el.value, this.delta = 0, this._removeChars();
        var b = g.parse(a);
        ...
        if (this.sel = k.get(this.el), this.val = this.el.value, this.delta = 0, this.sel.begin !== this.sel.end)this.delta = -1 * Math.abs(this.sel.begin - this.sel.end), this.val = l.removeChars(this.val, this.sel.begin, this.sel.end); else if (b)if (b && 46 == b) {
        ...
        } else b && this.sel.begin - 1 >= 0 && (this.val = l.removeChars(this.val, this.sel.end - 1, this.sel.end), this.delta = -1);
        ...
        this.newPos = this.sel.end + this.delta, this._removeChars(), this._validateInpts(), this._addChars(), this.el.value = this.val.substr(0, this.mLength), k.set(this.el, this.newPos)
        ...
        for (var a = 0, b = 0; b <= this.mLength; b++) {
            ...
            f = b >= this.sel.begin ? f + this.delta : f, c = this.val.charAt(f), (d && d == c || e && e == c) && (this.val = l.removeChars(this.val, f, f + 1), a--)
        }
        this.hldrs = {}, this.focus = this.val.length
    }, d.prototype._validateInpts = function () {
        for (var a = 0; a < this.val.length; a++) {
            var b = this.inpts[a];
            f[b] && f[b].test(this.val.charAt(a)) || this.inpts[a] && (this.val = l.removeChars(this.val, a, a + 1), this.focusStart--, this.newPos--, this.delta--, a--)
        }
    }, d.prototype._addChars = function () {
        if (this.opts.persistent) {
            for (var a = 0; a <= this.mLength; a++)this.val.charAt(a) || (this.val = l.addChars(this.val, this.opts.placeholder, a), this.hldrs[a] = this.opts.placeholder), this._addChar(a);
            for (; this.chars[this.focus];)this.focus++
        } else for (var b = 0; b <= this.val.length; b++)this._addChar(b)
    }, d.prototype._addChar = function (a) {
        var b = this.chars[a];
        return b ? (l.isBetween(a, [this.sel.begin - 1, this.newPos + 1]) && (this.newPos++, this.delta++), a <= this.focus && this.focus++, this.delta < 0 && this.val.charAt(a) == b ? !0 : (this.hldrs[a] && (delete this.hldrs[a], this.hldrs[a + 1] = this.opts.placeholder), this.val = l.addChars(this.val, b, a), void 0)) : !0
    };
    var g = {}, h = 4, i = new RegExp("{{([^}]+)}}", "g"), j = function (a) {
        for (var b, c = []; b = i.exec(a);)c.push(b);
        return c
    };
    g.parse = function (a) {
        ...
        for (g; d > g; g++)g == c[e].index ? i(c[e][1]) : b.chars[g - e * h] = a.charAt(g);
    ...
    a.fn[o] = function (b) {
        ...
        }), this.resetPattern = function (b) {
            return this.each(function () {
                var c = a.data(this, "plugin_" + o);
                c && c.resetPattern(b)
            }), this
        }, this
    }
